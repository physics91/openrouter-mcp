...
class TaskDecomposer:
        ...
        return {
            ...
            TaskType.CREATIVE: {
                "common_patterns": [
                    {
                        "pattern": "brainstorm",
                        "capabilities": [ModelCapability.CREATIVITY],
                    },
                    {
                        "pattern": "generate",
                        "capabilities": [ModelCapability.CREATIVITY],
            ...
            TaskType.ANALYSIS: {
                "common_patterns": [
                    ...
                    {
                        "pattern": "summarize",
                        "capabilities": [ModelCapability.ACCURACY],
                    },
                ],
    ...
    async def decompose_task(self, task: TaskContext) -> EnsembleTask:
        ...
        ensemble_task = EnsembleTask(
            task_id=task.task_id, original_task=task, decomposition_strategy=strategy
        )
        ...
        logger.info(
            f"Decomposed task {task.task_id} into {len(sub_tasks)} sub-tasks using {strategy.value} strategy"
        )
        ...
        for i, category in enumerate(categories):
            ...
            for j, sub_category in enumerate(sub_categories):
                sub_task = SubTask(
                    ...
                    task_type=task.task_type,
                    required_capabilities=self._get_subcategory_capabilities(
                        sub_category, task.task_type
                    ),
...
class ModelAssigner:
    ...
    async def _assign_single_model(
        self, sub_task: SubTask, available_models: List[ModelInfo]
    ) -> ModelAssignment:
    ...
    def _calculate_model_score(self, model: ModelInfo, sub_task: SubTask) -> float:
        ...
        final_score = (
            capability_score * 0.5 + availability_factor * 0.2 + accuracy_factor * 0.3
        ) * priority_multiplier
    ...
    def _generate_assignment_justification(
        self, model: ModelInfo, sub_task: SubTask, score: float
    ) -> str:
...
class EnsembleReasoner(CollectiveIntelligenceComponent):
    ...
    async def process(self, task: TaskContext, **kwargs) -> EnsembleResult:
        ...
        try:
            ...
            logger.info(
                f"Ensemble reasoning completed for task {task.task_id} in {processing_time:.2f}s"
            )
    ...
    async def _execute_sequential(
        ...
        for sub_task in ensemble_task.sub_tasks:
            ...
            if not result.success and sub_task.priority == TaskPriority.CRITICAL:
                logger.warning(
                    f"Critical sub-task {sub_task.sub_task_id} failed, stopping execution"
                )
                break
    ...
    def _find_assignment(
        self, sub_task_id: str, assignments: List[ModelAssignment]
    ) -> ModelAssignment:
    ...
    async def _aggregate_results(
        self, ensemble_task: EnsembleTask, sub_task_results: List[SubTaskResult]
    ) -> EnsembleResult:
    ...
    def _synthesize_final_content(
        self, successful_results: List[SubTaskResult], ensemble_task: EnsembleTask
    ) -> str:
    ...
    def _calculate_performance_metrics(
        self, sub_task_results: List[SubTaskResult], total_time: float
    ) -> PerformanceMetrics:
