...
class TestRunner:
    ...
    def run_linting(self) -> Dict[str, int]:
        ...
        # Black
        print("Running black...")
        results["black"] = subprocess.call(["black", "--check", "--diff", "src/", "tests/"])

        # isort
        print("Running isort...")
        results["isort"] = subprocess.call(["isort", "--check-only", "--diff", "src/", "tests/"])

        # flake8
        print("Running flake8...")
        results["flake8"] = subprocess.call(["flake8", "src/", "tests/"])

        # pylint
        print("Running pylint...")
        results["pylint"] = subprocess.call(["pylint", "src/"])

        # mypy
        print("Running mypy...")
        results["mypy"] = subprocess.call(["mypy", "src/"])
