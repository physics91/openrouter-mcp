...
class TestTaskDecomposer:
    ...
    def test_get_phase_capabilities(self):
        ...
        # Test analysis phase
        analysis_caps = decomposer._get_phase_capabilities(
            "analyze the problem", TaskType.REASONING
        )
        ...
        impl_caps = decomposer._get_phase_capabilities(
            "implement code", TaskType.CODE_GENERATION
        )
        assert ModelCapability.CODE in impl_caps

        # Test validation phase
        val_caps = decomposer._get_phase_capabilities(
            "test and validate", TaskType.CODE_GENERATION
        )
...
class TestModelAssigner:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_assign_models_multiple_subtasks(
        ...
        assert all(
            isinstance(assignment, ModelAssignment) for assignment in assignments
        )
    ...
    @pytest.mark.unit
    def test_generate_assignment_justification(
        self, mock_model_provider, sample_models
    ):
        ...
        justification = assigner._generate_assignment_justification(
            model, sub_task, score
        )
...
class TestEnsembleReasoner:
    ...
    @pytest.mark.unit
    def test_synthesize_final_content(
        ...
        for i, (result, priority) in enumerate(
            zip(sample_processing_results[:3], priorities)
        ):
            sub_task = SubTask(
        ...
        final_content = reasoner._synthesize_final_content(
            sub_task_results, ensemble_task
        )
        ...
        for result in sub_task_results:
            assert any(
                part in final_content for part in result.result.content.split()[:3]
            )
    ...
    async def test_ensemble_performance_with_many_subtasks(
        self, performance_mock_provider
    ):
    ...
    async def test_ensemble_with_all_subtask_failures(self, mock_model_provider):
        ...
        mock_model_provider.process_task.side_effect = Exception(
        ...
        assert (
            "Unable to complete task" in result.final_content
    ...
    async def test_concurrent_ensemble_processing(self, mock_model_provider):
        ...
        results = await asyncio.gather(
            *[reasoner.process(task) for task in tasks], return_exceptions=True
        )
