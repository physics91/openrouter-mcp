...
class TestConsensusEngine:
    ...
    def test_consensus_engine_initialization(
        self, mock_model_provider, consensus_config
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_select_models_basic(
        self, mock_model_provider, sample_task, consensus_config
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_select_models_respects_exclusions(
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_get_model_responses_success(
        self, mock_model_provider, sample_task, consensus_config
    ):
        ...
        responses = await engine._get_model_responses(
            sample_task, model_ids, request_id
        )
    ...
    async def test_get_model_responses_timeout_handling(self, sample_task):
        ...
        with pytest.raises(ValueError, match="Insufficient responses"):
            await engine._get_model_responses(
                sample_task, ["slow_model"], "test_timeout_request"
            )

    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_majority_vote_consensus(
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_confidence_threshold_consensus(
        self, mock_model_provider, sample_task, sample_processing_results
    ):
        """Test confidence threshold consensus building."""
        config = ConsensusConfig(
            strategy=ConsensusStrategy.CONFIDENCE_THRESHOLD, confidence_threshold=0.8
        )
        engine = ConsensusEngine(mock_model_provider, config)
        ...
        for i, result in enumerate(sample_processing_results[:3]):
            result.confidence = (
                0.9 if i == 0 else 0.7
            )  # Only first model meets threshold
    ...
    async def test_full_consensus_process(
        self, mock_model_provider, sample_task, consensus_config
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_model_reliability_updates(
        self, mock_model_provider, sample_task, consensus_config
    ):
    ...
    def test_calculate_agreement_level(self, mock_model_provider):
        ...
        assert (
            engine._calculate_agreement_level(0.7) == AgreementLevel.MODERATE_CONSENSUS
        )
    ...
    @pytest.mark.unit
    def test_group_similar_responses(
        ...
        assert all(
            all(isinstance(response, ModelResponse) for response in group)
            for group in groups
        )
    ...
    def test_calculate_consensus_confidence(
        ...
        confidence = engine._calculate_consensus_confidence(
            consensus_group, all_responses
        )
    ...
    @pytest.mark.unit
    def test_model_relevance_calculation(
        self, mock_model_provider, sample_models, sample_task
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_consensus_with_different_strategies(
        self, mock_model_provider, sample_task
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_error_handling_insufficient_models(
        self, failing_model_provider, sample_task
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_consensus_history_tracking(
        self, mock_model_provider, sample_task, consensus_config
    ):
    ...
    @pytest.mark.unit
    def test_model_reliability_scores_access(
        self, mock_model_provider, consensus_config
    ):
        ...
        test_scores = {"model1": 0.85, "model2": 0.92, "model3": 0.78}
    ...
    async def test_concurrent_consensus_requests(
        ...
        results = await asyncio.gather(
            *[engine.process(task) for task in tasks], return_exceptions=True
        )
    ...
    async def test_consensus_performance_benchmark(
    ...
    async def test_empty_model_response_handling(
        self, mock_model_provider, sample_task
    ):
