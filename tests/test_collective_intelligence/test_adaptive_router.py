...
class TestPerformancePredictor:
    ...
    def test_calculate_complexity_factor(self, sample_task):
        ...
        simple_task = TaskContext(
            task_id="simple", task_type=TaskType.FACTUAL, content="What is 2+2?"
        )
...
class TestAdaptiveRouter:
    ...
    async def test_full_routing_process(self, mock_model_provider, sample_task):
        ...
        assert decision.selected_model_id in [
            model.model_id for model in await mock_model_provider.get_available_models()
        ]
    ...
    async def test_evaluate_models(
        ...
        evaluations = await router._evaluate_models(
            sample_task, sample_models, RoutingStrategy.ADAPTIVE
        )
    ...
    async def test_evaluate_single_model(
        ...
        evaluation = await router._evaluate_single_model(
            sample_task, model, RoutingStrategy.QUALITY_OPTIMIZED
        )
    ...
    def test_configure_routing(self, mock_model_provider):
        ...
        router.configure_routing(decision_timeout=10.0, exploration_rate=0.2)
    ...
    async def test_concurrent_routing_requests(self, mock_model_provider):
        ...
        decisions = await asyncio.gather(
            *[router.process(task) for task in tasks], return_exceptions=True
        )
