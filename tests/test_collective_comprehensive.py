...
class CollectiveIntelligenceTestSuite:
    ...
    def create_task_context(
        self, content: str, task_type: str = "reasoning"
    ) -> TaskContext:
        ...
        return TaskContext(
            task_type=task_type_enum, content=content, requirements={}, constraints={}
        )
    ...
    async def test_collective_chat_completion(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.client:
                ...
                success = bool(
                    result.consensus_content and len(result.consensus_content) > 50
                )
                ...
                return {
                    ...
                    "consensus_response": (
                        result.consensus_content[:200] + "..."
                        if len(result.consensus_content) > 200
                        else result.consensus_content
                    ),
    ...
    async def test_ensemble_reasoning(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.client:
                ...
                success = bool(
                    result.final_content and len(result.sub_task_results) > 0
                )
                ...
                return {
                    ...
                    "final_result": (
                        ...
                        if len(result.final_content) > 200
    ...
    async def test_adaptive_model_selection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.client:
                ...
                success = bool(
                    decision.selected_model_id and decision.confidence_score > 0
                )
                ...
                return {
                    ...
                    "selection_reasoning": (
                        decision.justification[:150] + "..."
                        if len(decision.justification) > 150
    ...
    async def test_collaborative_problem_solving(self) -> Dict[str, Any]:
        ...
        try:
            ...
            task.requirements = {
                "stakeholders": ["restaurants", "grocery_stores", "consumers"]
            }
            ...
            async with self.client:
                ...
                response = await self.client.chat_completion(
                    ...
                    messages=[
                        {
                            "role": "user",
                            "content": "Say 'API integration test successful'",
                        }
                    ],
                    max_tokens=50,
                )
    ...
    async def run_comprehensive_test_suite(self) -> Dict[str, Any]:
        ...
        if integration_result["success"]:
            ...
            print(
                "[ERROR] OpenRouter API integration failed - skipping collective intelligence tests"
            )
    ...
    def generate_test_summary(
        self, test_results: List[Dict[str, Any]], total_time: float
    ) -> Dict[str, Any]:
        """Generate comprehensive test summary."""
        successful_tests = [r for r in test_results if r["success"]]
        failed_tests = [r for r in test_results if not r["success"]]

        avg_response_time = sum(r["response_time"] for r in test_results) / len(
            test_results
        )
        ...
        summary = {
            ...
            "tools_status": {
                "collective_chat_completion": next(
                    (
                        r["success"]
                        for r in test_results
                        if r["test_name"] == "collective_chat_completion"
                    ),
                    False,
                ),
                "ensemble_reasoning": next(
                    (
                        r["success"]
                        for r in test_results
                        if r["test_name"] == "ensemble_reasoning"
                    ),
                    False,
                ),
                "adaptive_model_selection": next(
                    (
                        r["success"]
                        for r in test_results
                        if r["test_name"] == "adaptive_model_selection"
                    ),
                    False,
                ),
                "cross_model_validation": next(
                    (
                        r["success"]
                        for r in test_results
                        if r["test_name"] == "cross_model_validation"
                    ),
                    False,
                ),
                "collaborative_problem_solving": next(
                    (
                        r["success"]
                        for r in test_results
                        if r["test_name"] == "collaborative_problem_solving"
                    ),
                    False,
                ),
                "openrouter_integration": next(
                    (
                        r["success"]
                        for r in test_results
                        if r["test_name"] == "openrouter_integration"
                    ),
                    False,
                ),
            },
...
async def main():
    ...
    try:
        ...
        for result in summary["individual_results"]:
            ...
            print(
                f"  {status_symbol} {result['test_name']}: {result['response_time']:.2f}s"
            )
