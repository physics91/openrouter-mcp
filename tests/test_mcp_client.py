...
async def test_collective_intelligence_tools():
    ...
    try:
        ...
        available_collective_tools = [
            tool["name"] for tool in tools if tool["name"] in collective_tools
        ]

        print(
            f"[SUCCESS] Found {len(available_collective_tools)} collective intelligence tools:"
        )
        ...
        if "collective_chat_completion" in available_collective_tools:
            ...
            try:
                ...
                print(
                    f"[SUCCESS] Collective Chat completed in {end_time - start_time:.2f}s"
                )
                    ...
                    if isinstance(content, list) and len(content) > 0:
                        ...
                        if response_data:
                            ...
                            try:
                                ...
                                print(
                                    f"[INFO] Consensus: {parsed.get('consensus_response', 'N/A')[:100]}..."
                                )
                                print(
                                    f"[INFO] Agreement: {parsed.get('agreement_level', 'N/A')}"
                                )
                                print(
                                    f"[INFO] Models: {len(parsed.get('participating_models', []))}"
                                )
        ...
        if "ensemble_reasoning" in available_collective_tools:
            ...
            try:
                ...
                print(
                    f"[SUCCESS] Ensemble Reasoning completed in {end_time - start_time:.2f}s"
                )
                if "content" in result:
                    content = result["content"]
                    if isinstance(content, list) and len(content) > 0:
                        ...
                        if response_data:
                            try:
                                parsed = json.loads(response_data)
                                print(
                                    f"[INFO] Final Result: {parsed.get('final_result', 'N/A')[:100]}..."
                                )
                                print(
                                    f"[INFO] Subtasks: {len(parsed.get('subtask_results', []))}"
                                )
                            except json.JSONDecodeError:
                                print(f"[INFO] Response: {response_data[:200]}...")
        ...
        if "adaptive_model_selection" in available_collective_tools:
            ...
            try:
                ...
                print(
                    f"[SUCCESS] Adaptive Model Selection completed in {end_time - start_time:.2f}s"
                )
                if "content" in result:
                    content = result["content"]
                    if isinstance(content, list) and len(content) > 0:
                        ...
                        if response_data:
                            try:
                                parsed = json.loads(response_data)
                                print(
                                    f"[INFO] Selected Model: {parsed.get('selected_model', 'N/A')}"
                                )
                                print(
                                    f"[INFO] Confidence: {parsed.get('confidence', 'N/A')}"
                                )
                            except json.JSONDecodeError:
                                print(f"[INFO] Response: {response_data[:200]}...")
        ...
        if "cross_model_validation" in available_collective_tools:
            ...
            try:
                ...
                print(
                    f"[SUCCESS] Cross-Model Validation completed in {end_time - start_time:.2f}s"
                )
                if "content" in result:
                    content = result["content"]
                    if isinstance(content, list) and len(content) > 0:
                        ...
                        if response_data:
                            try:
                                parsed = json.loads(response_data)
                                print(
                                    f"[INFO] Validation Result: {parsed.get('validation_result', 'N/A')}"
                                )
                                print(
                                    f"[INFO] Score: {parsed.get('validation_score', 'N/A')}"
                                )
                            except json.JSONDecodeError:
                                print(f"[INFO] Response: {response_data[:200]}...")
        ...
        if "collaborative_problem_solving" in available_collective_tools:
            ...
            try:
                ...
                print(
                    f"[SUCCESS] Collaborative Problem Solving completed in {end_time - start_time:.2f}s"
                )
                if "content" in result:
                    content = result["content"]
                    if isinstance(content, list) and len(content) > 0:
                        ...
                        if response_data:
                            try:
                                parsed = json.loads(response_data)
                                print(
                                    f"[INFO] Final Solution: {parsed.get('final_solution', 'N/A')[:100]}..."
                                )
                                print(
                                    f"[INFO] Strategy: {parsed.get('strategy_used', 'N/A')}"
                                )
                            except json.JSONDecodeError:
                                print(f"[INFO] Response: {response_data[:200]}...")
