...
class TestAdvancedBenchmarkHandler:
    ...
    def enhanced_handler(self):
        ...
        mock_cache.get_model_info = Mock(
    ...
    async def test_parallel_benchmarking(self, enhanced_handler):
        ...
        with patch.object(enhanced_handler, "benchmark_model") as mock_benchmark:
            ...
            comparison = await enhanced_handler.benchmark_models_parallel(
                models=models, prompt="Test parallel execution", max_concurrent=3
            )
...
class TestNewBenchmarkTools:
    ...
    @pytest.mark.skip(
        reason="compare_models_weighted method not implemented - uses compare_models instead"
    )
    @pytest.mark.asyncio
    async def test_compare_model_performance_tool(self):
        ...
        weighting = {"speed": 0.25, "cost": 0.25, "quality": 0.4, "throughput": 0.1}
        ...
        assert all(
            metric in result["detailed_scores"]
            for metric in ["speed", "cost", "quality", "throughput"]
        )
...
class TestBenchmarkIntegration:
    ...
    @pytest.mark.skip(
        reason="Test uses unimplemented methods (export_to_csv, compare_models_weighted)"
    )
    @pytest.mark.asyncio
    async def test_end_to_end_enhanced_benchmarking(self):
        ...
        analysis_result = analyzer.compare_models_weighted(
            metrics, {"speed": 0.3, "cost": 0.3, "quality": 0.4}
        )
